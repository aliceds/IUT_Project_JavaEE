...
public class CommandeAdminServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet CommandeGestion</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet CommandeGestion at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        String attribut = (String) request.getParameter("i");
        //String commande = "SELECT * FROM commande where idClient=(select idClient from utilisateur where pseudo='"+pseudo+"')";
        ResultSet res = null;
        try {
            res = statement.executeQuery("SELECT * FROM commande where idClient=" + attribut);
        ...
        try {
            while (res.next()) {
                ...
                java.util.Date dateD = formatter.parse(debut);
        ...
        this.getServletContext().getRequestDispatcher("/WEB-INF/gestionCommandeAdmin.jsp").forward(request, response);
    }
